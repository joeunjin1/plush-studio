...
export const partSchema = z.object({
  ...
  trims: z.string().max(160).default(""),
  process: z.string().max(160).default("봉제"),
  toleranceMm: z.number().min(0).max(50).default(2),
...
export function createProjectFromTemplate(templateId: TemplateId): Project {
  ...
  return {
    ...
    assets: [],
    decals: [],
    parts: templateParts(templateId, color).map(part => ({
      ...part,
      id: newRequestId(),
      fabric: "원단 지정",
      trims: "",
