...
## 2026-09-10 — Official product buyer-order staging UAT

The staging-only `reference_product_order_requests` contract was applied through migration 013. An initial buyer submit attempt failed before request creation because the authenticated caller lacked `USAGE` on the private `reference_order_private` function schema. Staging repair migration 014 granted schema usage to `authenticated` only; `anon` and `public` remain revoked. No Production database, Auth configuration, existing role, or existing request record was modified.

After the repair, an authenticated buyer completed a Bernese memorial-tag order with quantity 100 and a stated delivery date. The customer screen returned request ID `ea24e2bc-2a05-4c33-88ca-e4a194dfdaaf` and clearly labelled the result as a production-review request rather than a payment or binding purchase. A staging read-only verification returned `matching_request_count = 1`, confirming no duplicate request after the retry.

The metadata-only verification returned source SKU `PS-BERNESE-MEMORIAL-001`, reference product version `v01`, owner-provided model version `v02`, `sha256-recorded`, personalization profile `memorial-tag-text-v01`, placement `기념택 앞면`, `text-metadata-only`, quantity 100, and request status `received`. The UAT query deliberately excluded personalisation text, buyer email, contact data, object key, URL, and all binary content. This validates the staging text-personalization path and request idempotency only. A real mobile submission, private image-logo upload/read isolation, buyer-to-buyer RLS isolation, lifecycle handoff, and all Production equivalents remain pending.
