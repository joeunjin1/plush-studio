...
The staging Supabase Redirect URL allowlist was updated to include `https://plush-studio-1jhf3sby7-joeunjin1s-projects.vercel.app` for the latest Preview UAT. The prior staging URL remains temporarily allowlisted for rollback; Production's authentication URL configuration was not opened or changed.
...
## 2026-09-07 staging-only owner GLB v02 record

GitHub staging commit `eb779a0` (`feat: register owner supplied Bernese GLB v02`) deployed successfully through Vercel Preview. It binds the Bernese reference product only in the staging browser configuration to the owner-supplied versioned catalog object `bernese-memorial-plush/3d/berner_plush_360_v02.glb`. The object MIME type is `model/gltf-binary`; the recorded 6,547,424-byte source checksum is `eeebf4aba5861150ae0eca006cf85bb7d0583e7b9400c15ff60ac2106ac647d5`. The owner confirmed normal visual rendering in the staging buyer preview.

This staging record is deliberately not a Production release. Migration 008 is applied in both environments, while catalog migrations 009, 010, and 011 remain staging-only. The Production catalog has not received the v02 object, no production URL mapping was changed, and no merge from `staging` to `main` occurred. Before any Production promotion, repeat the SQL migration, asset upload, object checksum, buyer-preview UAT, email redirect, and five-view fallback checks in the isolated Production environment after separate owner approval.
